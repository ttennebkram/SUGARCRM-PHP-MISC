...
//        $count_query = $seed->create_list_count_query($main_query);
	 ...
	 * @return array
	 */
	protected function _ORIG_performSearch(
	    $query,
	    $modules,
	    $offset = -1
	    )
	...
	protected function _performSearch($query, $modules, $offset = -1) {
		$url = "http://192.168.15.112:8888/solr/collection1/select?q=" . urlencode($query) . "&sort=module+asc&rows=20&wt=php";
		eval ('$response = ' . file_get_contents($url) . ';');
		
		$solrDocs = $response["response"]["docs"];
		$sugarDocs = array();
		
		$pageData = array();
		$pageData["offsets"] = array();
		$pageData["offsets"]["current"] = -1;
		$pageData["offsets"]["next"] = -1;
		$pageData["offsets"]["prev"] = -1;
		$pageData["offsets"]["end"] = -1;
		$pageData["offsets"]["total"] = 0;
		$pageData["offsets"]["totalCounted"] = 1;
		$pageData["bean"] = array();
		$pageData["bean"]["objectName"] = "Account";
		$pageData["bean"]["moduleDir"] = "Accounts";
		$readAccess = 4;
		
		$currentModule = "";
		foreach ( $solrDocs as $solrDoc ) {
		    $newModule = $solrDoc["module"];

		    if ( $newModule != $currentModule ) {
		        $moduleHive = array();
		        $moduleHive["data"] = array();
		        $moduleHive["pageData"] = $pageData;
				$moduleHive["readAccess"] = $readAccess;
				
		        # Now add this hive to main results
		        $sugarDocs[ $newModule ] = $moduleHive;
		    }
		    $currentModule = $newModule;

		    # step 1: init a new doc
		    $sugarDoc = array();
		    
		    # step 2: copy over the good stuff
			# TODO: aux_name in Solr - not sure where this goes?
		    $sugarDoc["ID"] = $solrDoc["id"];
		    $sugarDoc["DESCRIPTION"] = $solrDoc["description"];
		    $sugarDoc["FIRST_NAME"] = $solrDoc["first_name"];
		    $sugarDoc["LAST_NAME"] = $solrDoc["last_name"];
		    # extra credit, it may display this, not sure this is proper concatenation syntax
		    $sugarDoc["FULL_NAME"] = $solrDoc["first_name"] . ' ' . $solrDoc["last_name"];
			
			if ($currentModule == "Contacts" || $currentModule == "Leads") {
				$sugarDoc["NAME"] = $solrDoc["first_name"] . ' ' . $solrDoc["last_name"];
			} else if ($currentModule == "Notes") {
				$sugarDoc["NAME"] = $solrDoc["description"];
			} else if ($currentModule == "Meetings" || $currentModule == "Bugs" || $currentModule == "Cases") {
				$sugarDoc["NAME"] = $solrDoc["company_name"] . ' - ' . $solrDoc["aux_name"];
			} else if ($currentModule == "Contracts" || $currentModule == "Projects" || $currentModule == "Project Tasks") {
				$sugarDoc["NAME"] = $solrDoc["aux_name"] . ' - ' . $solrDoc["description"];
			} else if ($currentModule == "Tasks" || $currentModule == "Opportunities") {
				$sugarDoc["NAME"] = $solrDoc["aux_name"];
			}
			else {
				$sugarDoc["NAME"] = $solrDoc["company_name"];
			}

		    # step 3: add this doc to the hive's doclist
		   	$sugarDocs[$currentModule]["data"][] = $sugarDoc;
		}
